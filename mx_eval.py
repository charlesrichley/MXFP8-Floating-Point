...
def FMA_multiply_add(a: int, b: int, c: int) -> int:
    ...
    if ab_exp > c_exp: 
        ab_man_shift = ab_mantissa << (ab_exp - c_exp)
        E4M3_sign = ab_sign
        if ab_sign != c_sign: 
            num = ab_man_shift - c_mantissa
        else: 
            num = ab_man_shift + c_mantissa

    elif c_exp > ab_exp: 
        E4M3_sign = c_sign
        c_man_shift = c_mantissa << (c_exp - ab_exp)
        if ab_sign != c_sign: 
            num = c_man_shift - ab_mantissa
        else: 
            num = c_man_shift + ab_mantissa
    else: 
        if ab_sign != c_sign: 
            if c_mantissa > ab_mantissa:
                E4M3_sign = c_sign
                num = c_mantissa - ab_mantissa 
            else: 
                E4M3_sign = ab_sign
                num = ab_mantissa - c_mantissa
        else: 
            num = c_mantissa + ab_mantissa
